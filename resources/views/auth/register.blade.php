...
<div id="addModal" class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 hidden">
    <div class="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form id="addForm" class="p-6 space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div><label class="text-sm font-medium">Position</label><input type="text" id="position" class="w-full border rounded-lg px-3 py-2"></div>
                <div><label class="text-sm font-medium">Department</label>
                    <select id="dept" class="w-full border rounded-lg px-3 py-2 bg-white">
            ...
            <div><label class="text-sm font-medium">Employment Status</label>
                <select id="empStatus" class="w-full border rounded-lg px-3 py-2">
...
<script>
    ...
    function loadData() {
        ...
        if (stored) {
            ...
            employees = [
                { id: "1", firstName: "Alice", lastName: "Chen", phone: "09123456789", viber: "9123456789", social: "", position: "UI Designer", dept: "Tech Engineering", status: "active", profilePic: null, signature: null, hasAccount: false, email: null, loginEnabled: false },
                { id: "2", firstName: "Robert", lastName: "Katz", phone: "09234567890", viber: "", social: "", position: "Sales Lead", dept: "Sales", status: "active", profilePic: null, signature: null, hasAccount: false, email: null, loginEnabled: false },
                { id: "3", firstName: "Maria", lastName: "Garcia", phone: "09345678901", viber: "9345678901", social: "fb.com/maria", position: "HR Manager", dept: "HR", status: "active", profilePic: null, signature: null, hasAccount: true, email: "maria@company.com", loginEnabled: true }
            ];
            saveData();
        }
        refreshUI();
    ...
    function renderTable() {
        ...
        tbody.innerHTML = pageData.map(emp => {
            const statusBadge = emp.status === 'active' ? 'bg-green-100 text-green-700' : (emp.status === 'on_leave' ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700');
            const statusText = emp.status === 'active' ? 'Active' : (emp.status === 'on_leave' ? 'On Leave' : 'Inactive');
            const avatar = emp.profilePic ? `<img src="${emp.profilePic}" class="w-9 h-9 rounded-full object-cover">` : `<div class="w-9 h-9 rounded-full bg-blue-500 text-white flex items-center justify-center text-sm font-bold">${(emp.firstName[0] || '')}${(emp.lastName[0] || '')}</div>`;
            const accountSection = emp.hasAccount 
                ? `<span class="text-xs bg-green-100 text-green-700 px-2 py-1 rounded-full"><i class="fas fa-check-circle mr-1"></i> Active</span>`
                : `<button onclick="openAccountModal('${emp.id}')" class="text-xs bg-purple-100 hover:bg-purple-200 text-purple-700 px-3 py-1 rounded-full transition"><i class="fas fa-user-plus mr-1"></i> Create Account</button>`;
            return `<tr class="hover:bg-gray-50">
                <td class="px-6 py-3 whitespace-nowrap"><div class="flex items-center gap-3"><div>${avatar}</div><div><div class="font-medium">${emp.firstName} ${emp.lastName}</div><div class="text-xs text-gray-400">${emp.phone || ''}</div></div></div></td>
                <td class="px-6 py-3">${emp.position || '—'}</td>
                <td class="px-6 py-3">${emp.dept || '—'}</td>
                ...
                <td class="px-6 py-3 space-x-2">
                    <button onclick="openEditModal('${emp.id}')" class="text-blue-500 hover:text-blue-700"><i class="fas fa-edit"></i></button>
                    <button onclick="deleteEmployee('${emp.id}')" class="text-red-500 hover:text-red-700"><i class="fas fa-trash"></i></button>
                    <button onclick="viewProfile('${emp.id}')" class="text-gray-500 hover:text-gray-700"><i class="fas fa-eye"></i></button>
                   <button onclick="disableEmployee('${emp.id}')" 
